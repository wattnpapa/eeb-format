...
// pako statt node:zlib: der Kern muss auch im Testlauf ohne Node-Kernmodule
// auskommen, damit derselbe Test unter jsdom laufen kann (ADR-003, Regel 2).
...
describe("Segment-Split/Merge-Roundtrip", () => {
  ...
  it("setzt Teile in beliebiger Scan-Reihenfolge zusammen", () => {
    const b = grosserBogen();
    const teile = segmentPayloadUrls(encodePayload(b, kompressor), 4).map(parseSegmentUrl);
    gleich(segmenteZuBogen([teile[2]!, teile[0]!, teile[3]!, teile[1]!], kompressor), b);
  });

  it("hält den Single-QR unverändert (kein Segment-Marker)", () => {
    const b = grosserBogen(2);
    const url = encodePayloadUrl(b, kompressor);
    expect(istSegmentNutzlast(url)).toBe(false);
    gleich(decodePayloadUrl(url, kompressor), b);
  });
});

describe("Segment-Fehlerfälle", () => {
  it("lehnt einen fehlenden Teil ab", () => {
    const teile = segmentPayloadUrls(encodePayload(grosserBogen(), kompressor), 3).map(parseSegmentUrl);
    expect(() => segmenteZuBogen([teile[0]!, teile[2]!], kompressor)).toThrow(/unvollständig|fehlt/i);
  });

  it("lehnt vermischte Teile zweier Bögen ab (andere id)", () => {
    const a = segmentPayloadUrls(encodePayload(grosserBogen(40), kompressor), 2).map(parseSegmentUrl);
    const b = segmentPayloadUrls(encodePayload(grosserBogen(41), kompressor), 2).map(parseSegmentUrl);
    expect(() => segmenteZuBogen([a[0]!, b[1]!], kompressor)).toThrow(/vermischt|unvollständig|Prüfsumme/i);
  });

  it("lehnt einen defekten Chunk über die Prüfsumme ab", () => {
    const teile = segmentPayloadUrls(encodePayload(grosserBogen(), kompressor), 2).map(parseSegmentUrl);
    const kaputt: SegmentTeil = { ...teile[1]!, chunk: teile[1]!.chunk.slice() };
    kaputt.chunk[0] = kaputt.chunk[0]! ^ 0xff;
    expect(() => segmenteZuBogen([teile[0]!, kaputt], kompressor)).toThrow(/Prüfsumme|EEB2/i);
  });
  ...
  it("verlangt mindestens 2 Teile", () => {
    expect(() => segmentPayloadUrls(encodePayload(grosserBogen(), kompressor), 1)).toThrow(/mindestens 2/i);
  });
});

describe("segmentSammeln (Scanner-Sammelzustand)", () => {
  function teileVon(anzahl: number): SegmentTeil[] {
    return segmentPayloadUrls(encodePayload(grosserBogen(), kompressor), anzahl).map(parseSegmentUrl);
  }
  ...
  it("sammelt Teile bis zur Vollständigkeit", () => {
    ...
    gleich(segmenteZuBogen(r3.teile, kompressor), grosserBogen());
  });
  ...
  it("beginnt bei einem fremden Teil neu", () => {
    ...
    const neu = segmentPayloadUrls(encodePayload(grosserBogen(41), kompressor), 2).map(parseSegmentUrl);
