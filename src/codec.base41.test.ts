...
describe("Abwärtskompatibilität: QR-Codes aus der Base64url-Zeit", () => {
  it("liest einen alten Einzelbogen-QR", () => {
    ...
    const alteUrl = EEB_URL_PREFIX + base64UrlKodieren(encodePayload(b, kompressor));
    gleich(decodePayloadUrl(alteUrl, kompressor), b);
  });

  it("liest eine alte Vorlagen-URL (Marker V. + Base64url)", () => {
    const b = bogen();
    const alteUrl = EEB_URL_PREFIX + EEB_VORLAGE_MARKER + base64UrlKodieren(encodePayload(b, kompressor));
    gleich(decodeVorlagePayloadUrl(alteUrl, kompressor), b);
  });
  ...
  it("liest alte Segment-QRs und setzt sie zusammen", () => {
    ...
    gleich(segmenteZuBogen(alteUrls.map(parseSegmentUrl), kompressor), b);
  });
  ...
  it("liest gemischte Sätze — alter und neuer Teil desselben Bogens", () => {
    ...
    gleich(segmenteZuBogen([parseSegmentUrl(neu[0]!), parseSegmentUrl(alterTeil2)], kompressor), b);
  });
});
...
describe("Neue QR-Inhalte", () => {
  ...
  it("Vorlage behält den Marker V. vor dem Base41-Marker", () => {
    const b = bogen();
    const url = encodeVorlagePayloadUrl(b, kompressor);
  ...
  it("Segment-Kopf bleibt parsebar, obwohl der Chunk selbst einen Punkt trägt", () => {
    ...
    gleich(segmenteZuBogen(urls.map(parseSegmentUrl), kompressor), b);
  });
